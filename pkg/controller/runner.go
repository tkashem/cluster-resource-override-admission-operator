...
func (r *runner) Run(parent context.Context, controller Interface, errorCh chan<- error) {
	...
	if parent == nil || controller == nil {
		errorCh<- errors.New("invalid input to Starter.Start")
		return
	}
	...
	klog.V(1).Infof("controller=%s starting informer", controller.Name())
	go controller.Informer().Run(parent.Done())

	klog.V(1).Infof("controller=%s waiting for informer cache to sync", controller.Name())
	...
	klog.V(1).Infof("controller=%s started %d worker(s)", controller.Name(), controller.WorkerCount())

	errorCh<- nil

	klog.V(1).Infof("controller=%s waiting", controller.Name())

	// Not waiting for any child to finish, waiting for the parent to signal done.
	<-parent.Done()

	klog.V(1).Infof("controller=%s shutting down queue", controller.Name())
}
