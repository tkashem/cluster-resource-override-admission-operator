...
func NewReconciler(options *handlers.Options) *reconciler {
	handlers := HandlerChain{
		...
		handlers.NewServiceCAConfigMapHandler(options),
		handlers.NewServiceHandler(options),
		handlers.NewServiceCertSecretHandler(options),
		handlers.NewCertReadyHandler(options),
		handlers.NewDeploymentHandler(options),
		handlers.NewDeploymentStatusHandler(options),
...
func (r *reconciler) Reconcile(request controllerreconciler.Request) (result controllerreconciler.Result, err error) {
	...
	observed, getErr := r.lister.Get(request.Name)
	if getErr != nil {
		klog.Errorf("key=%s failed to get object - %s", request.Name, getErr.Error())
	...
	desired, result, _, err := r.handlers.Handle(reconcileContext, copy)

	updateErr := r.updater.Update(observed, desired)
	if updateErr != nil {
		klog.Errorf("key=%s failed to update status - %s", request.Name, updateErr.Error())

		if err != nil {
			err = fmt.Errorf("reconciliation error - %s -- update status error - %s", err.Error(), updateErr.Error())
			return
		}
