...
func run(config *operator.Config) {
	// TODO: bin/operator flag redefined: log_dir
	// https://stackoverflow.com/questions/37284423/glog-flag-redefined-error
	// klog.InitFlags(nil)
	...
	go func() {
		...
		<-shutdownHandler
		klog.V(1).Info("Received SIGTERM or SIGINT signal, initiating shutdown.")
	}()

	klog.V(1).Infof("operator configuration - %s", config)
	klog.V(1).Info("starting the operator")

	errorCh := make(chan error, 0)
	runner := operator.NewRunner()
	go runner.Run(config, errorCh)
	if err := <-errorCh; err != nil {
		klog.V(1).Info("error running operator - %s", err.Error())
	}

	klog.Infof("operator is running, waiting for the operator to be done.")
	<-runner.Done()
